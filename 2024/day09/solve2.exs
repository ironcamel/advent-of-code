defmodule Main do
  ...
  def fix_disk(disk) do
    max_i = disk |> Map.keys() |> Enum.max()

    Enum.reduce(max_i..1//-1, disk, fn i, acc ->
      orig_data = Enum.reject(disk[i], fn val -> val == "." end)
      data = acc[i]
      open_i = Enum.find(0..(i - 1), fn j -> free_space(acc[j]) >= length(orig_data) end)

      if open_i do
        new_data = acc[open_i]
        free = free_space(new_data)
        new_data = Enum.reject(new_data, fn val -> val == "." end)
        new_data = new_data ++ orig_data ++ List.duplicate(".", free - length(orig_data))
