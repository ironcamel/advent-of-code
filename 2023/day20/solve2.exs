defmodule Main do
  def main() do
    ...
    1..1_000_000_000
    |> Enum.reduce(modules, fn i, acc ->
      {acc, signals} = push_button(acc, listeners)
      try do
        process(signals, acc)
      catch
        name -> IO.puts "found #{name} at #{i}"
        exit(:normal)
      end
    end)
  end
  ...
  def handle_signal(modules, {val, from, to}) do
    ...
    _names = ["ph", "vn", "kt", "hn"]
    modules =
      #if val == 0 and to == "ph" do
      #if val == 0 and to == "vn" do
      #if val == 0 and to == "kt" do
      if val == 0 and to == "hn" do
        throw to
      else
        modules
      end
...
# 244151741342687 - input-large.txt answer
